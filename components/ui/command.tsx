  ...
  return (
    ...
    <div data-slot="command-input-wrapper" className="border-b border-hair">
      <CommandPrimitive.Input
        ...
        className={cn(
          "w-full bg-transparent px-4 py-3.5 text-[0.9375rem] text-fg outline-hidden placeholder:text-fg-3 disabled:cursor-not-allowed disabled:opacity-50",
          className,
        )}
        {...props}
      />
...
function CommandEmpty({
  ...
  return (
    <CommandPrimitive.Empty
      ...
      className={cn("px-4 py-6 text-center text-[0.8125rem] leading-[1.6] text-fg-3", className)}
      {...props}
    />
  );
}
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "group/command-item relative flex cursor-default items-baseline gap-3 px-4 py-2 text-[0.875rem] text-fg-2 outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-[selected=true]:bg-raised data-[selected=true]:text-fg",
        className,
      )}
      {...props}
    >
...
function CommandShortcut({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        "ml-auto text-[0.6875rem] tracking-[0.06em] text-muted-foreground group-data-selected/command-item:text-foreground",
        className,
      )}
      {...props}
    />
