    ...
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          ...
          // Adapted: the popup genuinely leaves the document, so it takes
          // `lift` instead of the shipped `shadow-md ring-1`, and the
          // motion joins the house ease (120/160ms, exponential out).
          className={cn(
            "z-50 max-h-(--available-height) min-w-44 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-none bg-popover p-0 text-popover-foreground lift duration-160 ease-expo outline-none data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:overflow-hidden data-closed:fade-out-0",
            className,
          )}
          {...props}
        />
...
function DropdownMenuItem({
  ...
  return (
    <MenuPrimitive.Item
      ...
      // Adapted: the shipped item is uppercase at the one label style.
      // Menu items are readable choices, so they take the UI step
      // (0.8125rem, sentence case) and the raised ground for focus.
      className={cn(
        "group/dropdown-menu-item relative flex cursor-default items-center gap-2.5 rounded-none px-3 py-2 text-[0.8125rem] leading-[1.55] font-normal tracking-normal text-fg-2 outline-hidden select-none focus:bg-raised focus:text-fg not-data-[variant=destructive]:focus:**:text-fg data-inset:pl-9.5 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive dark:data-[variant=destructive]:focus:bg-destructive/20 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5 data-[variant=destructive]:*:[svg]:text-destructive",
        className,
      )}
      {...props}
    />
