...
- **Display Small** (600, 1.875rem, 1.16, -0.026em): The same title below 640px, and the Courses list's h1 — one step below the Course page's own Display, so a list never speaks as loudly as the page it opens. One of the system's responsive steps; the Display XL set and Display Large are the others, and all live at the top of the ramp where a line has room to be wrong.
...
The landing's scroll motion runs on Motion (`motion/react` in `components/landing-motion.tsx`). A greyscale hairline tracks reading progress at the viewport top with a spring. Each walk step (`components/landing-walk.tsx`) settles once through `Reveal` (16px rise, 560ms); the outline rows assemble with a short clip and 8px settle staggered by 65ms, and the lesson exercise wipes in through a clip path with a spring. All Motion work keeps content visible by default so a failed script leaves a readable page; the CSS entrance animations are unchanged and carry the reduced-motion exception the user requested.

The landing now uses a guided walk in place of the living course demonstration and the separate quality, workflow, learning-support, and details sections. This section supersedes the earlier landing-specific composition and motion descriptions.
...
Visitors choose Photography, Jazz harmony, or Databases with pressed-state buttons in a sticky bar given room to breathe, followed by a spaced anchor index to each step. The selected example reads one step per scroll section — the goal with its starting point, the outline with its modules, one lesson exercise, help and changes, then the begin facts — alternating left and right down the page at wide widths. Each section carries one ground, divided by hairlines. Examples are labeled illustrative. Steps are separated by generous whitespace with no enclosing borders, rounded cards, or shadows; mono numerals carry the sequence and the section's one olive mark sits on the outline approval. The subject selector uses a neutral bottom rule for its selected state, and the step index is plain anchor text with a panel hover.

Scrolling settles each step once; changing subjects replays the step's assemble through the same shared state. Anchor links jump between the five steps; sections stack in reading order at every width, and subject controls remain reachable from the sticky bar.

The walk carries the quality, workflow, learning-support, and begin copy in condensed form: sequencing and review inside the goal and lesson steps, Tutor and Tailor inside help and changes, Depth, Course Language, and generation time inside begin. The primary hero action is "Create your course" and uses the existing Google sign-in flow; the closing action repeats it verbatim with the Google mark, so both ends of the page carry the same CTA.

The navbar is fixed to the viewport top: a compact translucent canvas bar with a backdrop blur and a bottom hairline. "Sign in" is a compact primary control carrying the Google mark at 16px. Navigation names the walk and the closing start. Anchor targets carry scroll margins that clear the bar. The footer carries "© 2026 Mikasa · Developed by Andrés Sanabria".

The walk's subject tabs join without gaps. Section headings carry no wayfinding triangle. The page sits over a notebook-paper ground: a faint ruled grid in the hairline token at 20% over canvas, holding still while content scrolls over it. It keeps animating nothing of its own and stays clear of reading text.
