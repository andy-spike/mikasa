// Step-only helper, imported dynamically from inside stepRepairFragments so
// the workflow build never resolves its server imports. Loading it at the top
// level of a workflow file would pull the AI SDK serde classes into the
// workflow manifest before tree-shaking (a false-positive serde warning).
